...
const WorldMapComponent = () => {
    ...
    return (
        <div className='relative flex justify-center align-center overflow-hidden rounded-[10px] text-center bg-[#d2f7ff] rounded-xl p-3 mb-3"'>
            <div className={`absolute inset-0 z-[5] pointer-events-none transition-opacity duration-700 ease-in-out 
                bg-[radial-gradient(circle_at_center,_transparent_10%,_rgba(0,0,0,0.5)_80%)] ${selectedId ? 'opacity-100' : 'opacity-0'}`}></div>
            <div className={`absolute left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-[2rem] font-bold z-[20] 
                ...
                {activeMaker && (
                    <div className="descriptionBox">
                        ...
                        <p className="break-keep overflow-wrap-anywhere whitespace-pre-wrap m-0 text-[18px] leading-[1.5] text-[#f1f2f6]">{activeMaker.description}</p>
            ...
            <div className="w-full h-full z-[1] relative transition-transform duration-[700ms] ease[cubic-bezier(0.4,0,0.2,1)]" style={{ transformOrigin: 'center', transform: `translate(${zoomStyle.x}%, ${zoomStyle.y}%) scale(${zoomStyle.scale})` }}>
                ...
                {markers.map((marker) => {
                    ...
                    return (
                        <div key={marker.id} className={`absolute w-[24px] h-auto cursor-pointer z-[5] -translate-x-1/2 -translate-y-1/2
